- an [[agora abstract]].
	- #push [[agora abstracts]]
		...
		- Some hypothesis that we are testing:
			- A knowledge commons can provide benefits to the community efficiently, as the cost of systemic integrations with a hub such as the Agora can scale with `O(N)` instead of the `O(N^2)` provided by a naive full mesh.
			-
		-