...
The Agora as described in this paper is just [[an Agora]], meaning it is just one possible implementation of the principles delineated here. Because [[Agora protocol]] tries to build on common principles[^commoning] and incorporate conventions already in use at the time of writing, other Agoras already exist (if not by name, in spirit.)
